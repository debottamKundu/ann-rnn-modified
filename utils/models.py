...
class BayesianActor(object):
    ...
    def reset(
        ...
        self.emission_probs = np.array(block_side_probs)
        ...
        self.curr_stim_posterior = np.full(shape=(num_sessions, 1, 2), fill_value=0.5)
        self.curr_block_posterior = np.full(shape=(num_sessions, 1, 2), fill_value=0.5)

        mu = np.sort(
            np.concatenate(
                [
                    np.array(possible_trial_strengths[1:]),
                    -1.0 * np.array(possible_trial_strengths),
                ]
            )
        )
    ...
    def __call__(self, model_input):
        ...
        # blank dt, skip
        if torch.all(model_input["stimulus"] == 0.0) and torch.all(
            model_input["reward"] == 0.0
        ):
            # print('Blank dt, skipping')
            pass
        # trial end, update block posterior
        elif torch.all(model_input["stimulus"] == 0.0) and torch.all(
            model_input["reward"] != 0.0
        ):
        ...
        model_output = dict(
            ...
            linear_output=torch.from_numpy(
                np.full_like(self.curr_stim_posterior, fill_value=np.nan)
            ),
            core_output=torch.from_numpy(
                np.full_like(self.curr_block_posterior, fill_value=np.nan)
            ),
        )
    ...
    def update_stim_posterior(self, stimulus):
        ...
        stim_side_strength_joint_prob = np.einsum(
            "us,bts->btus", self.prob_mu_given_stim_side, self.curr_stim_posterior
        )
        ...
        diff_obs_likelihood = scipy.stats.norm.pdf(
            ...
            loc=self.mu,
            scale=np.sqrt(2) * np.ones_like(self.mu),
        )  # scale is std dev
        ...
        diff_obs_stim_side_joint_prob = np.sum(
            diff_obs_stim_side_strength_joint_prob, axis=3
        )
        assert len(diff_obs_stim_side_joint_prob.shape) == 4

        # normalize by p(o_{<=t})
        # shape = (num of observations, # of time steps, # of obs = 1)
        diff_obs_marginal_prob = np.sum(diff_obs_stim_side_joint_prob, axis=3)
    ...
    def update_block_posterior(self, correct_stim_side):
        ...
        if self.start:
            ...
            curr_joint_prob = np.multiply(
                self.emission_probs[correct_stim_side], self.curr_block_posterior
            )
            self.start = False
        else:
            curr_joint_prob = np.multiply(
                ...
                np.einsum(
                    "ib,stb->sti", self.transition_probs, self.curr_block_posterior
                ),
            )
...
class BayesianBlocklessActor(BayesianActor):
    ...
    def update_block_posterior(self, correct_stim_side):
        ...
        self.curr_block_posterior = np.full(shape=(1, 1, 2), fill_value=0.5)
...
class ExponentialWeightedActor(object):
    ...
    def reset(
        ...
        self.exp_decaying_stimulus_prior = np.full(
            shape=(num_sessions, 1, 2), fill_value=0.5
        )
    ...
    def __call__(self, model_input):
        ...
        model_output = dict(
            ...
            core_output=torch.from_numpy(
                np.full_like(self.exp_decaying_stimulus_prior, fill_value=np.nan)
            ),
        )
    ...
    def update_stim_prior(self, correct_stim_side):
        ...
        # I think it is
        self.exp_decaying_stimulus_prior[:, :, correct_stim_side] += 1.0 - self.decay

        # check that this is valid probability distribution
        assert np.all(np.isclose(self.exp_decaying_stimulus_prior.sum(axis=2), 1.0))
...
class RecurrentModel(nn.Module):
    ...
    def __init__(self, model_architecture, model_kwargs):
        ...
        self.core = self._create_core(
            model_architecture=model_architecture, model_kwargs=model_kwargs
        )

        masks = self._create_connectivity_masks(
            model_str=model_architecture, model_kwargs=model_kwargs
        )
    ...
    def _create_core(self, model_architecture, model_kwargs):
        ...
        core = core_constructor(
            input_size=self.input_size, batch_first=True, **model_kwargs["core_kwargs"]
        )
        ...
        if param_init_str == "default":
            ...
            raise NotImplementedError(
                f"Weight init function {param_init_str} unrecognized"
            )
    ...
    def _create_mask(self, mask_type_str, output_shape, input_shape):
        ...
        if mask_type_str == "none":
            ...
            first_column[: int(0.2 * output_shape)] = 1.0
            ...
            graph = networkx.watts_strogatz_graph(
                n=output_shape, k=int(0.2 * output_shape), p=0.1
            )
    ...
    def apply_connectivity_masks(self):
        ...
        self.readout.weight.data[:] = torch.mul(self.readout.weight, self.readout_mask)
