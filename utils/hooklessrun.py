...
def convert_session_data_to_ibl_changepoint_csv(
    session_data, env_block_side_probs, log_dir
):
    ...
    stim_probability_left = block_side_probs[
        ((trial_end_data.block_side.values + 1) / 2).astype(np.int)
    ]

    # should be either 0, 0.0625, 0.125, 0.25, 1
    contrast = trial_end_data.trial_strength.values
    contrast_map = {0: 0, 0.5: 0.0625, 0.75: 0.125, 1.0: 0.25, 1.5: 1.0}
    ...
    assert np.all(
        (np.sign(position) == np.sign(response_choice))[trial_correct.astype(np.bool)]
    )
    ...
    ibl_changepoint_df.to_csv(os.path.join(log_dir, "MIT_001.csv"), index=False)


def create_logger(run_dir):

    logging.basicConfig(
        filename=os.path.join(run_dir, "logging.log"), level=logging.DEBUG
    )
...
def create_optimizer(model, optimizer_params):
    ...
    optimizer = optimizer_constructor(
        params=model.parameters(), **optimizer_params["kwargs"]
    )
...
def extract_session_data(envs):
    ...
    session_data["signed_trial_strength"] = (
        session_data["trial_strength"] * session_data["trial_side"]
    )

    session_data["concordant_trial"] = (
        session_data["trial_side"] == session_data["block_side"]
    )

    # record whether dt was in correct trial
    session_data["correct_trial_dt"] = np.nan
    for _, trial in session_data.groupby(
    ...
    proj_trial_sides_onto_block_sides = (
        np.dot(block_sides, trial_sides)
        * block_sides
...
def run_envs(model, envs, log_results=True):
    ...
    action_taken_by_total_trials = session_data[
        session_data.trial_end == 1.0
    ].action_taken.mean()

    correct_action_taken_by_action_taken = session_data[
        session_data.action_taken == 1.0
    ...
    dts_by_trial = (
        session_data.groupby(["session_index", "block_index", "trial_index"])
        .size()
        .mean()
    )
    ...
    if log_results:
        ...
        logging.info(
            f"# Action Trials / # Total Trials: " f"{action_taken_by_total_trials}"
        )

        logging.info(
            f"# Correct Trials / # Action Trials: "
            f"{correct_action_taken_by_action_taken}"
        )

        logging.info(
            f"# Correct Trials / # Total Trials: "
            f"{correct_action_taken_by_total_trials}"
        )

        logging.info(
            f"# Correct Trials / # Total Steps: "
            f"{correct_action_taken_by_total_steps}"
        )

        logging.info(f"Average steps per trial: " f"{dts_by_trial}")
...
def stitch_plots(log_dir):
    ...
    stitched_plots_path = os.path.join(
        log_dir, "analyze_" + log_dir.split("/")[1] + ".pdf"
    )
